...
| QLT-01 scores  ✅ M2 | Per page: blur (Laplacian variance inside quad), glare (% clipped highlights), shadow (luma gradient across page), crop completeness (quad confidence + edge contact), perspective (quad skew), readability (share of OCR words under confidence threshold). iOS 26: lens-smudge signal. Thresholds set on the benchmark corpus, per document class. |
| QLT-02 duplicates/sequence  ✅ M2 | Feature-print distance between pages → "possible duplicate"; page-number patterns in OCR ("3 de 7", "Page 3 of 7") → "page 4 may be missing / out of order". Warn only, never auto-fix. |
